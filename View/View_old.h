...
template<          >                                 struct RecalcStrides<EmptyList>                 { using type = EmptyList; };
template<size_t D,  size_t S>                        struct RecalcStrides<List<P<D,  S >, EmptyList>>{ using type = List<P<D, S>, EmptyList>;      static const size_t carry = D*S; };
