...
template<typename H, typename T> struct Length<List<H, T>        >{ static const int length = Length<T>::length + 1; };
template<typename H            > struct Length<List<H, EmptyList>>{ static const int length = 1; };
