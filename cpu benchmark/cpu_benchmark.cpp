...
void invoke() {
    ...
    summary("CPU Blocked 4", { functional_view_blocked<n, 4 >(A, B), t1::test<n>(A,B), t2::test<n,4 >(A,B), t4::test<n,4>(A,B),  t6::test<n,4>(A,B) });
    summary("CPU Blocked 8", { functional_view_blocked<n, 8 >(A, B), t1::test<n>(A,B), t2::test<n,8 >(A,B), t4::test<n,8>(A,B),  t6::test<n,8>(A,B) });
    summary("CPU Blocked 16",{ functional_view_blocked<n, 16>(A, B), t1::test<n>(A,B), t2::test<n,16>(A,B), t4::test<n,16>(A,B), t6::test<n,16>(A,B) });
    summary("CPU Blocked 32",{ functional_view_blocked<n, 32>(A, B), t1::test<n>(A,B), t2::test<n,32>(A,B), t4::test<n,32>(A,B), t6::test<n,32>(A,B) });
}

int main() {
    //std::cout << "           func_view_blocked     t1          t2          t4              t5\n";
    /*invoke<64>();
    invoke<128>();
    invoke<256>();*/
    //invoke<512>();
	...
	auto naive_res = naive(A, B);
	std::cout << "Naive: " << naive_res.second << " ms\n";

    auto ref = functional_view_blocked<n, 16>(A, B);
	std::cout << "Reference: " << ref.second << " ms\n";

	auto ref2 = view_blocked<n, 16>(A, B);
	std::cout << "Reference2: " << ref2.second << " ms\n";

	//auto q = testone_lvl5<n, n/16>(A, B, ref.first)(Int<0>());
	//std::cout << "Test6: " << q.dt << " (" << (q.passed ? '+' : '-') << ")\n";
	
	std::vector<Result> vres;
	fold_imap(Int<120/2-1>(), [](auto s, Result const& res)
	{
		s->push_back(res);
		return s;
	}, &vres, testone_lvl5<n, n/16>(A, B, ref.first));

	std::sort(vres.begin(), vres.end(), [](auto const& r1, auto const& r2){ return r1.dt < r2.dt;});
	std::cout << "\n";
	for(auto const& e : vres)
	{
		std::cout << e.index << " : " << e.dt << " ms " << e.dt / ref.second * 100 << "% (" << (e.passed ? '+' : '-') << ")\n";
	}
	system("PAUSE");
	return 0;
}
